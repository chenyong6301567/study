...
public class DistributedLock {
	...
	public boolean getLock() throws KeeperException, InterruptedException {
		...
		LOG.info(LOG_PREFIX_OF_THREAD + "尝试获取锁,创建selfPath路径:" + selfPath);
		if (checkMinPath()) {
			return true;
		}
		return false;
	...
	public void unlock() {
		try {
			...
			LOG.info(LOG_PREFIX_OF_THREAD + "删除本节点,释放锁：" + selfPath);
			zk.close();
	...
	public boolean checkMinPath() throws KeeperException, InterruptedException {
		...
		String str = selfPath.substring(GROUP_PATH.length() + 1);
